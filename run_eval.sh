...
torchrun --nproc_per_node=8 python eval_sspro_hf_dp.py \
  --model_dir "$MODEL_DIR" \
...
torchrun --nproc_per_node=8 python eval_ssv2_hf_dp.py \
  --model_dir "$MODEL_DIR" \
...
torchrun --nproc_per_node=8 python eval_osworldg_hf_dp.py \
  --model_dir "$MODEL_DIR" \
...
torchrun --nproc_per_node=8 python eval_uivision_hf_dp.py \
  --model_dir "$MODEL_DIR" \
